...
public final class ICU {
	...
	// Returns the appropriate Locale given a String of the form returned by
	// toString. This is very lenient, and doesn't care what's between the
	// underscores: this method can parse strings that Locale.toString won't
	// produce. Used to remove duplication.
	...
	public static Locale localeFromString(String localeName) {
		...
		if (first == -1) {
			...
			return new Locale(localeName.substring(0, first), localeName.substring(first + 1, second),
					localeName.substring(second + 1));
		}
	}

	public static Locale[] localesFromStrings(String[] localeNames) {
		// We need to remove duplicates caused by the conversion of "he" to
		// "iw", et cetera.
		// Java needs the obsolete code, ICU needs the modern code, but we let
		// ICU know about
	...
	// Returns the ICU version in use. This is "4.4" for gingerbread, for
	// example.

	public static native String getIcuVersion();

	// Returns the Unicode version our ICU supports. This is "5.2" for
